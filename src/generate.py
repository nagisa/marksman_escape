...
class Trie(object):
    ...
    def build_matcher(self, f):
        if len(list(self.keys())) > BRANCH_FACTOR:
            ...
            self.write(f, "}", -1)
        else: # Don’t generate too many branch, it hurts compile times hard
            f.write("""match &name[{}..] {{\n""".format(self.depth))
...
if __name__ == "__main__":
    with open("unescape_named.rs", "w") as f:
        ...
        # get_named_ref generation
        f.write("/// Get the unicode value of a named reference.\n")
        f.write("pub fn get_named_ref(name: &[u8]) -> Option<&'static [u8]> {\n")
        ...
        # generate and test whether all the references are decoded correctly
